...
export default function SingItOut() {
  ...
  const [timeLeft, setTimeLeft] = useState(30); // New state for time left
  ...
  const handleAccept = () => {
    ...
    generateLetter();
    setTimeLeft(30); // Reset time left to 30 seconds
  };
  ...
  useEffect(() => {
    if (gameStarted) {
      clearTimeout(timerId); // Clear the old interval
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
      ...
      <p className="mb-4 text-lg">Time Left: {timeLeft <= 10 ? <span className="text-red-500">{timeLeft}</span> : timeLeft}</p>
